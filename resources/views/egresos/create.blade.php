        ...
        <form id="formEgreso" action="{{ route('egresos.store') }}" method="POST">
            ...
            <div class="mt-6">
                ...
                <div id="casillasContainer">
                    @php
                        $oldPresupuestos = old('presupuesto_id', ['']);
                        $oldValores = old('valor', ['']);
                    @endphp

                    @foreach ($oldPresupuestos as $i => $oldPresupuesto)
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                            <select name="presupuesto_id[]" class="form-control" required>
                                <option value="">Seleccione una casilla</option>
                                @foreach ($casillas as $casilla)
                                    <option value="{{ $casilla->id }}" {{ $oldPresupuesto == $casilla->id ? 'selected' : '' }}>
                                        {{ $casilla->nombre_casilla }}
                                    </option>
                                @endforeach
                            </select>
                            <input type="number" name="valor[]" class="form-control valor-casilla" placeholder="Valor" required min="0" value="{{ $oldValores[$i] ?? '' }}">
                        </div>
                    @endforeach
                </div>
...
<script>
    function confirmarEnvio() {
        // Validar que el primer select y valor tengan datos
        const firstSelect = document.querySelector('select[name="presupuesto_id[]"]');
        const firstValor = document.querySelector('input[name="valor[]"]');
        if (!firstSelect.value || !firstValor.value) {
            alert("Debe seleccionar una casilla y poner un valor antes de guardar.");
            return;
        }
