...
<div class="container py-5">
    <div class="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-6">
        ...
        <form action="{{ route('egresos.update', $movimiento->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div id="casillas-container">
                @foreach($egresos as $index => $egreso)
                    <div class="grid grid-cols-2 gap-4 mb-3 border p-3 rounded-md bg-gray-50">
                        <!-- id del egreso para mapear exacto en el update -->
                            ...
                            <select name="presupuesto_id[]" class="form-control" required>
                                ...
                                @foreach($casillas as $casilla)
                                    <option value="{{ $casilla->id }}"
                                        {{ $egreso->presupuesto_id == $casilla->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="valor[]" class="form-control"
                                value="{{ old('valor.' . $index, number_format($egreso->valor, 2, '.', '')) }}" required>
                        </div>
                    </div>
                @endforeach
            </div>
